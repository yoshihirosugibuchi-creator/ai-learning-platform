...
export default function SkpHistoryPage() {
  ...
  const getSourceBadgeColor = (source: string) => {
    if (source.startsWith('quiz_session_') || source === 'quiz_completion') {
      return 'bg-blue-100 text-blue-800'
    } else if (source.startsWith('course_session_') || source.startsWith('course_completion_')) {
      return 'bg-green-100 text-green-800'
    } else if (source.startsWith('streak_')) {
                          ...
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            >
                              {transaction.source === 'quiz_completion' ? 'クイズ' :
                               ...
                               transaction.source === 'achievement' ? '達成' :
                               transaction.source.startsWith('quiz_session_') ? 'クイズ' :
                               transaction.source.startsWith('course_session_') ? 'コース' :
                               transaction.source.startsWith('course_completion_') ? 'コース完了' :
                               transaction.source.startsWith('streak_') ? '継続ボーナス' : transaction.source}
                            </Badge>
                          </div>
