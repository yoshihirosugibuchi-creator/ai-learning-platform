...
export async function GET(request: Request) {
  ...
  const userId = searchParams.get('userId') || '2a4849d1-7d6f-401b-bc75-4e9418e75c07'
  
  console.log('🔍 Debugging daily_xp_records table for user:', userId.substring(0, 8) + '...')
  ...
  try {
    ...
    return NextResponse.json({
      summary: {
        ...
        totalRecordsCount: allRecords?.length || 0,
        quizSessionsCount: quizSessions?.length || 0
      },
      tableAccessError: tableError?.message,
      userRecordsError: userError?.message,
      allRecordsError: allError?.message,
      quizSessionsError: quizError?.message,
      userRecords: userRecords || [],
      allRecords: allRecords || [],
      recentQuizSessions: quizSessions?.map(s => ({
