    ...
    for (const table of tables) {
      try {
        ...
        if (table === 'quiz_answers') {
          // quiz_answersは全削除（user_idがNULLのものも含む）
          deleteCondition = { column: 'id', operator: 'gt', value: 0 }
        } else {
          // その他は user_id != '' で削除
          deleteCondition = { column: 'user_id', operator: 'neq', value: '' }
        }
        ...
        if (deleteCondition.operator === 'gt') {
          query.gt(deleteCondition.column, deleteCondition.value)
        } else {
          query.neq(deleteCondition.column, deleteCondition.value)
        }
