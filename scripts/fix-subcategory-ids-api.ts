/**
 * APIを使用してサブカテゴリーIDを修正するスクリプト
 * ALEカテゴリー一覧PDFに基づいて正しい英語IDに修正
 */
...
async function fixSubcategoryIdsViaAPI() {
  console.log('🔄 APIを使用してサブカテゴリーIDの修正を開始します...')
  ...
  try {
    ...
    for (const subcategory of currentSubcategories) {
      ...
      if (correctId && correctId !== subcategory.subcategory_id) {
        ...
        // 編集APIを使用してIDを更新
        const updateResponse = await fetch(`http://localhost:3000/api/admin/subcategories/${subcategory.subcategory_id}/edit`, {
          method: 'PATCH',
          headers: {
        ...
        if (!updateResponse.ok) {
          ...
          console.error(`❌ 更新失敗 "${subcategory.subcategory_id}": ${errorData.error}`)
        } else {
          updateCount++
          console.log(`✅ 更新成功: "${correctId}"`)
        }
      } else {
        skipCount++
        if (!correctId) {
    ...
    console.log(`\n✅ 修正完了! 更新: ${updateCount}件, スキップ: ${skipCount}件`)

    // 3. 修正後の状況を確認
    const verifyResponse = await fetch('http://localhost:3000/api/subcategories')
    if (verifyResponse.ok) {
      const verifyData = await verifyResponse.json()
      console.log('\n📋 修正後のサブカテゴリーID一覧:')
      verifyData.subcategories?.forEach((sub: any) => {
