...
- ノードラベルは1行12文字以内（日本語全角基準）。超える場合は<br/>で改行
  ❌ A["秘密領域: 自分は知っている 他者は知らない"]（はみ出す）
  ✅ A["秘密領域<br/>自分は知っている<br/>他者は知らない"]
- \\nでの改行は禁止（パースエラーの原因。必ず<br/>を使う）
- ノードラベル内の絵文字は禁止（パースエラーの原因）
- 絵文字はsubgraphラベルのみ使用OK
- block-betaは使用禁止（レンダリング不具合あり）。表形式はflowchartのsubgraphで代替
