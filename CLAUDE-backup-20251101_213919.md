...
**最終更新**: 2025年10月30日  
**緊急追加**: ロジック削除絶対禁止・クライアント/サーバー分離標準導入

---

## 🚨 **【EMERGENCY WARNING】絶対に犯してはならない重大犯罪（2025.10.30追加）**

### **❌ 絶対禁止: デバッグ・修正時のロジック削除・無効化**

```markdown
🚨 **NEVER DELETE LOGIC - 絶対にロジックを削除するな**

❌ 禁止行為:
- console.log() でロジックを無効化
- 関数の中身を空にする
- return文で早期終了してロジックを迂回
- コメントアウトで機能を無効化
- 「簡単な」代替処理に置き換える

✅ 正しい対応:
- サーバーサイドロジック → API エンドポイントに移動
- クライアントサイドロジック → API呼び出しに変更
- 認証が必要な処理 → 認証付きAPI経由で実行
- データベースアクセス → 適切なクライアント/サーバー分離

🚨 違反すると: 重要機能の完全停止・要件未達成・開発プロジェクト失敗
```

### **📋 クライアント/サーバー分離の絶対標準（2025.10.30策定）**

#### **🖥️ クライアントサイド（Reactコンポーネント・lib/）で実行すべきこと**
```typescript
✅ 許可される処理:
- UI状態管理・イベントハンドリング
- フォーム入力検証・表示ロジック
- useStateやuseEffectによる画面制御
- 純粋関数による計算（データベースアクセスなし）
- fetch() によるAPI呼び出し
- NEXT_PUBLIC_SUPABASE_ANON_KEY 使用のSupabaseクライアント
- ローカルストレージ・セッションストレージ操作

❌ 絶対禁止:
- supabaseAdmin の使用・インポート
- SUPABASE_SERVICE_ROLE_KEY への直接アクセス
- 認証なしでの機密データベースアクセス
- サーバー環境変数の直接参照
- RLSをバイパスするような処理
```

#### **🖥️ サーバーサイド（/app/api/*・サーバー専用lib/）で実行すべきこと**
```typescript
✅ 許可される処理:
- データベースの直接操作（supabaseAdmin使用）
- 機密データの処理・計算
- 認証トークンの検証・権限チェック
- 複雑なビジネスロジック
- 外部API連携（API Key使用）
- ファイルシステムアクセス
- SUPABASE_SERVICE_ROLE_KEY 使用の管理者操作

❌ 絶対禁止:
- UI状態の直接操作
- Reactフック（useState、useEffect等）の使用
- ブラウザAPI（localStorage等）の使用
- Next.js App Routerのクライアント専用機能
```

#### **🔄 分離作業時の必須プロセス**
```markdown
Step 1: 現在のコードが何をしているか完全理解
Step 2: 機能を保持したままサーバーサイドAPIに移動
Step 3: クライアントサイドをAPI呼び出しに変更
Step 4: 認証・エラーハンドリングの適切な実装
Step 5: 元の機能と同等の動作確認

🚨 絶対守ること: 機能削除・簡略化は一切行わない
```
...
#### **Step 5: 既存エイリアス型保持（必須・完全版）**

**🚨 重要**: 以下は**完全な44個の型リスト**です。必ず全てを追加してください。

```bash
# 🔍 現在のエイリアス型定義を確認（デバッグ用）
echo "=== 現在のエイリアス型定義 44個 ==="
grep "export type.*Database\['public'\]" lib/database-types-official.ts | wc -l
grep "export type.*Database\['public'\]" lib/database-types-official.ts

# 🚨 完全な44個のエイリアス型を新しいファイルに追加
echo "
...
# ✅ エイリアス型の数を確認（44個であることを確認）
ALIAS_COUNT=$(grep "export type.*Database\['public'\]" lib/database-types-official-new.ts | wc -l)
echo "✅ エイリアス型定義追加完了: ${ALIAS_COUNT}個"
if [ "$ALIAS_COUNT" -ne 44 ]; then
  echo "⚠️ 警告: 型定義が44個ではありません (実際: ${ALIAS_COUNT}個)"
fi
```
...
### **🔄 型定義エイリアス管理（永続化対策・2025.10.31追加）**

#### **🚨 問題の背景**
- **問題**: 型定義再生成時に一部のエイリアス型（`DailyXPRecord`, `DailyAnalyticsBatchLog`, `WisdomCardMaster`等）が漏れる
- **原因**: CLAUDE.mdの保持リストが不完全（38個 vs 実際44個）
- **解決策**: 完全な44個リストの維持と新規追加時の同期管理

#### **新しい型エイリアスを追加する際の必須手順**

```bash
# 🔍 Step 1: 新しいエイリアス型を database-types-official.ts に追加
echo "export type NewTypeName = Database['public']['Tables']['new_table']['Row']" >> lib/database-types-official.ts

# 📝 Step 2: CLAUDE.md の Step 5 完全リストにも同じ型を追加
# （CLAUDE.mdファイルを直接編集し、44個→45個に更新）

# ✅ Step 3: 型数の確認
CURRENT_COUNT=$(grep "export type.*Database\['public'\]" lib/database-types-official.ts | wc -l)
echo "現在の型定義数: ${CURRENT_COUNT}個"

# 📋 Step 4: CLAUDE.md の数字も更新
# "完全な44個の型リスト" → "完全な${CURRENT_COUNT}個の型リスト" に変更

echo "⚠️ 重要: 型定義追加時は database-types-official.ts と CLAUDE.md の両方を更新すること"
```

#### **型定義漏れの予防チェック**

```bash
# 🔍 定期チェック用コマンド（型定義再生成前に実行推奨）
echo "=== 型定義漏れ予防チェック ==="

# 現在のファイルの型数
CURRENT_TYPES=$(grep "export type.*Database\['public'\]" lib/database-types-official.ts | wc -l)
echo "📊 現在のファイル: ${CURRENT_TYPES}個"

# CLAUDE.mdの型数
CLAUDE_TYPES=$(grep -c "export type.*Database\['public'\]" CLAUDE.md)
echo "📋 CLAUDE.mdリスト: ${CLAUDE_TYPES}個"

# 差分チェック
if [ "$CURRENT_TYPES" -eq "$CLAUDE_TYPES" ]; then
  echo "✅ 型定義数一致: 再生成時の漏れリスクなし"
else
  echo "⚠️ 型定義数不一致: 再生成時に漏れる可能性があります"
  echo "   対処: CLAUDE.md の Step 5 を現在のファイルに合わせて更新してください"
fi

# 具体的な差分を表示
echo "=== 現在のファイルで使用されている型（参考用） ==="
grep "export type.*Database\['public'\]" lib/database-types-official.ts | head -10
```
...
### ✅ DO (必須):
1. **database-types-official.ts** への型追加
2. **CLAUDE.md Step 5** への同じ型追加
3. **型数カウントの更新** (44個→新しい数)
4. **両方のファイルの同期確認**

### ❌ DON'T (禁止):
1. database-types-official.ts のみの更新
2. CLAUDE.md のみの更新
3. 型数の不一致を放置
4. 再生成前の同期確認省略

### 📋 チェックリスト:
- [ ] 新しい型を database-types-official.ts に追加
- [ ] 同じ型を CLAUDE.md Step 5 リストに追加
- [ ] CLAUDE.md の型数表記を更新 (44個→新しい数)
- [ ] 上記予防チェックコマンドで一致確認
- [ ] TypeScript/ESLint エラー0確認
```
