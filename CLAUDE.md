...
## 🚨 **バグ調査の鉄則（2026年1月30日追加 - 最高優先度）**

### **❌ 禁止事項**
- デバッグログやalertを繰り返し追加して原因を探る
- 「動くはず」という推測で修正を試みる
- 部分的なコード確認だけで結論を出す

### **✅ 必須：UI→API→DB整合性チェック**

バグが発生した場合、**最初に**以下のデータフローを確認すること：

```
1. UI側: どの値を送信しているか？
   - フォームの値、state、propsを確認
   - 例: question_type = 'multiple_choice'

2. API側: どの値を受け取り、どう処理しているか？
   - リクエストボディの型定義を確認
   - バリデーションロジックを確認
   - 例: step.question_type をそのままDBに渡している

3. DB側: どの値を許可しているか？
   - テーブル定義のCHECK制約を確認
   - NOT NULL制約、外部キー制約を確認
   - 例: CHECK (question_type IN ('single', 'multiple', 'ordering', 'text', 'hybrid'))

4. 不整合を発見 → 修正
   - UI: 'multiple_choice' → DB: 'multiple' のみ許可
   - → API側で変換処理を追加
```

### **📋 チェックリスト（バグ調査開始時に必ず実行）**

1. [ ] UI側で送信している値を特定
2. [ ] API側の受け取り・処理ロジックを確認
3. [ ] DB側の制約（CHECK, NOT NULL, FK）を確認
4. [ ] 値の不整合がないか照合
5. [ ] 不整合があれば変換処理またはバリデーションを追加

**この手順を踏まずにデバッグログを追加することは禁止。**
